...
//				AND cntl.url = '".$lang_url."'		
	...
	private function view()
	{
		...
		$sql = "SELECT cl.name country_name, cl.url country_url, catl.name as category_name, CONCAT(pcatl.url, '/', catl.url) as category_url,
		...
		WHERE s.id = ".$school['id'];	
			...
			$nav = $this->db->db_get_row($sql);

			$this->Navigator = '<span class="nav"></span><a href="/">Главная</a> > <a href="/'.langUrl($_SESSION['lang_url']).'category/'.$nav[3].'">'.$nav[2].
				'</a> > <a href="'.$nav[1].'">'.$nav[0].'</a> > <a href="'.langUrl($_SESSION['lang_url']).$nav[5].'">'.$nav[4].'</a> >'.$school['name'];
		}		
		else {
