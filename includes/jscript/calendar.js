...
calendar = {
	...
	month_names: ["Cічень","Лютий","Березень","Квітень","Травень","Червень","Липень","Серпень","Вересень","Жовтень","Листопад","Грудень"],
	...
	makeCalendar:function(year, month, day) {
		...
		this.wrt("<div class=\"calendar-body\"><table>");
		this.wrt("<tr><th><a href='javascript:calendar.makeCalendar("+(previous_month_year)+","+(previous_month)+");' title='"+this.month_names[previous_month]+" "+(previous_month_year)+"'><b>&laquo;</b></a></th>");
		...
		this.wrt("</select></th>");
		this.wrt("<th>&nbsp;&nbsp;<a href='javascript:calendar.makeCalendar("+(next_month_year)+","+(next_month)+");' title='"+this.month_names[next_month]+" "+(next_month_year)+"'><b>&raquo;</b></a></th></tr>");
		this.wrt("<tr class='header'>");
		for(var weekday=0; weekday<7; weekday++) 
			if ( weekday > 4)
				this.wrt("<td class='weekend'>"+this.weekdays[weekday]+"</td>");
			else
		...
		this.wrt("</table>");
		this.wrt("<input type='button' value='Закрить' class='calendar-cancel' onclick='calendar.hideCalendar();' /></div>");
