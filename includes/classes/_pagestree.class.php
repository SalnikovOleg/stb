...
class PagesTree extends Module
{
		...
		public function LoadContent()
		{
			...
			// текуший ID
			$sql = 'SELECT a.id, a.parent FROM '.$this->table.' a	WHERE a.url = "'.$this->query[$this->last].'" AND a.lang = '.$_SESSION['lang_id'].' AND a.deleted = 0';
		...
		protected function LoadPage($url)
		{
			...
			if ($page !== null)
			{
				...
				$this->tpl->assign('text', stripslashes($page['text']));
		...
		protected function GetNode($cId, $level, $preUrl, $condition, $page_enable = false)
		{
			...
			for ($i=0; $i<count($ds); $i++)
			{
				...
				if ($ds[$i]['node'] == 1)
				{
					$this->tree .= '<li><a '.$class.' href="'.$url.'" title="'.$ds[$i]['name'].'">'.substr($ds[$i]['name'],0,60)."</a>\n";				
					$this->tree .="\n<ul>\n";
		...
		{
			$ds = $this->db->get_tree_node_by_id($cId, $this->table, ' ORDER BY n.insert_date DESC');
			...
			for ($i=0; $i<count($ds); $i++)
			{
				...
				if ($ds[$i]['node'] == 1)
				{
					$this->tree .= '<li><a class="node" href="'.$url.'" title="'.$ds[$i]['name'].'">'.$ds[$i]['name']."</a>\n";				
					$this->tree .="\n<ul>\n";
					$this->GetContentNode($ds[$i]['id'], $level + 1, $preUrl.$ds[$i]['url'].'/');
					$this->tree .="</ul>\n";
				}
				else
					$this->tree .= '<li><a href="'.$url.'" title="'.$ds[$i]['name'].'">'.$ds[$i]['name']."</a>\n
	...
	private function GetItem($id)
	{
		if ($id != 0)
		{
			$sql = "SELECT n.id, n.parent, n.url, n.node, n.`name`, n.page_id, n.insert_date, n.disabled, n.selected, n.lang, IFNULL(n.image,'') AS image, n.image_alt,
				np.title, np.text, IFNULL(np.meta_title, nm.meta_title) as meta_title, IFNULL(np.meta_description, nm.meta_description) as meta_description,
...
//--------------------------- save  data  ---------------------------------
	...
	public function insert(&$data)
	{
		...
		if ($data['ispage'] == 1)
		{
			$sql = "INSERT INTO ".$this->table."_pages (`title`, `text`, `meta_title`, `meta_description`, `meta_keywords`, `description`) VALUES (
