...
/* класс от которого наследуются все модули выводящие контент */ 
 abstract class Module
 {
	...
	function __construct(&$db, &$params)
	{
		...
		$this->module_url = (isset($this->Params['url']))?$this->Params['url']:'';
		$this->realModuleUrl = (isset($this->Params['url']))?$this->Params['url']:'';
		$this->module_id =(isset($this->Params['id']))?$this->Params['id']:0;
	...
	{
		if ( !isset($this->query[MODULEINDEX]) || $this->query[MODULEINDEX]!= $this->realModuleUrl ) return null;
	...
	public function searchText($searchString)
	{
		...
		" WHERE MATCH(p.title, p.text) AGAINST('".$searchString."')  > 0.95 AND p.lang = ".$_SESSION['lang_id'];
